...
plot_saveif_ip <- function(df, opportunity, average, t_quartile, pod_colour){
...
plot_saveif_other <- function(df, opportunity, average, t_quartile, pod_colour){
...
#   
#   data[df[2] == "Low", 2] = chprop(myCellProps, background.color = rag_colours[6])
#   data[df[2]== "-", 2] = chprop(myCellProps, background.color = alpha(rag_colours[4], 0.2))
#   data[df[2] == "High", 2] = chprop(myCellProps, background.color = rag_colours[4])
#   
#   
...
plot_saveif_ip <- plot_saveif_ip(final_saveif_ip, opportunity, average, t_quartile, ip_colour)
plot_saveif_ae <- plot_saveif_other(final_saveif_ae, opportunity, average, t_quartile, ae_colour)
plot_saveif_op <- plot_saveif_other(final_saveif_op, opportunity, average, t_quartile, op_colour)
...
conditional_stp <- function(df){
  ...
  data[df[i] == "Low", i]  = chprop(myCellProps, background.color = rag_colours[6])
  data[df[i] == "-", i]    = chprop(myCellProps, background.color = alpha(rag_colours[4], 0.2))
  data[df[i] == "High", i] = chprop(myCellProps, background.color = rag_colours[4])
...
flex_rates_ip <- conditional_stp(final_rates_ip)
flex_rates_ae <- conditional_stp(final_rates_ae)
flex_rates_op <- conditional_stp(final_rates_op)

flex_roc_ip <- conditional_stp(final_roc_ip)
flex_roc_ae <- conditional_stp(final_roc_ae)
flex_roc_op <- conditional_stp(final_roc_op)
